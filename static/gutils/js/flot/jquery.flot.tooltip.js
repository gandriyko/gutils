		...
		plot.hooks.bindEvents.push(function (plot, eventHolder) {
			...
			$(placeholder).bind("plothover", function (event, pos, item) {
				if (item) {					
					...
					if(typeof(to.formater) == 'function') {
						tipText = to.formater(item.series.label, item.datapoint[0], item.datapoint[1])
					}
					else if(opts.xaxis.mode === "time" || opts.xaxes[0].mode === "time") {
